...
**Текущий фокус:** этапы 0–16 закрыты.
...
- [x] Этап 14 — Тон рекомендаций (ФТ-7): тишина на разовый treat, частотная рамка на паттерн, промпты 1.2 + eval
- [x] Этап 15 — Уведомление администратора о завершении курса
- [x] Этап 16 — Алерты админу о сбое рекомендаций и вечерней сводки
...
ФТ-14
> Реализовано: `topProducts` — топ-5 продуктов через `AIEngine.extractTopProducts` (один вызов на финальный отчёт). Вода не входит, остальные напитки (чай, кофе и т.п.) входят. Если модель недоступна или вернула мусор — эвристика без единиц измерения и без воды. `topProblems` — топ-5 по ключевым словам (сладкое, фастфуд, перекус и т.д.).
7.7 [x]
...
Этап 14. Тон рекомендаций (ФТ-7)
#	Шаг	Связь с ФТ
14.1 [x]
Обновить спецификацию тона: ФТ-7, SA_SUMMARY, evening-summary, CJM, vision
ФТ-7, ФТ-24
> Эталон «я заметил, что вы ели булочку» убран. Разовое удовольствие (алкоголь/десерт/джанк) — без рекомендации; повтор того же класса ≥ 2 за 7 локальных дней — частотная рамка, не замена на воду/протеин. Мгновенная рекомендация не пересказывает только что введённую запись. Вечерняя сводка может перечислить день.
14.2 [x]
История за 7 локальных дней в промпте анализа + пометка текущей записи
ФТ-6, ФТ-7
> `buildHistorySummary` отдаёт описания записей за окно (не только счётчики) и явно говорит модели, что текущая запись только что введена. Классификация treat — `packages/ai/src/treat-pattern.ts`.
14.3 [x]
Промпты 1.2, критерий treat_frequency, mock-эвристика
ФТ-7
> `PROMPT_VERSION = 1.2`. Analysis/recommendation/evening разделены: мгновенный ответ без пересказа, сводка может обзор дня. `MockAIEngine` молчит на разовый treat и отдаёт `treat_frequency` на паттерн.
14.4 [x]
Eval тона: тишина / частота / запрещённые формулировки
ФТ-7
> `packages/ai/src/recommendation-tone.eval.ts` — baseline MockAIEngine (пиво 1×, пиво 2×, курага) и контракт промпта OpenAICompatible. Запуск: `npm run eval`.

Этап 15. Уведомление администратора о завершении курса
#	Шаг	Связь с ФТ
15.1 [x]
Сводка админу в Telegram сразу после генерации финального Report
ФТ-13, ФТ-14, ФТ-19
> `notifyAdminAboutCourseCompletion` в `packages/scheduler/src/services/admin-course-summary.ts`, вызов из `completeCourse`. Канал — `sendAdminAlert` / `ADMIN_ALERT_TELEGRAM_ID`. Сбой отправки не блокирует сообщения клиенту.
15.2 [x]
Короткая выжимка: клиент + ключевые ответы анкеты
ФТ-14
> ФИ, Telegram, курс, период; из анкеты: цель, возраст, вес, что мешает, режим питания, активность, здоровье. Для продлённого enrollment берутся ответы последней completed-анкеты клиента.
15.3 [x]
Краткий отчёт о питании (не копия клиентского сообщения)
ФТ-14
> Заполняемость дневника, средняя калорийность, % соблюдения, топ продуктов и проблем. Без поздравления и AI-резюме для клиента.

Этап 16. Алерты админу, если рекомендация или вечерняя сводка не собрались
#	Шаг	Связь с ФТ
16.1 [x]
Классификация ошибок AI-провайдера (квота/биллинг, ключ, rate limit, недоступность)
nonFR §6
> `packages/shared/src/errors/provider-error.ts`. Текст алерта для человека, без сырого JSON: «закончились деньги на счёте AI-провайдера».
16.2 [x]
Алерт, если рекомендация не создана после записи дневника
ФТ-6, nonFR §6
> `processDiaryEntry` ловит сбой анализа/записи, логирует `AI_ENGINE_ERROR` и шлёт `notifyAdminJobFailure`. Лимит 2–3/день и пустые proposals не алертят.
16.3 [x]
Алерт, если вечерняя сводка не собралась; дайджест за прогон job
ФТ-24, nonFR §6
> Job собирает сбои в `notifyAdminJobFailuresDigest`. Fallback на эвристику при `provider_error` тоже будит админа (клиент получил запасной текст). Недоставка Telegram не дублируется (ФТ-21). Дедуп `JOB_FAILURE_ALERT_INTERVAL_MINUTES`.
