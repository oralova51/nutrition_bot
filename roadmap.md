...
**Текущий фокус:** Этап 13 — тестовое покрытие: шаги 13.1–13.6 завершены (инфраструктура, доставка в Telegram, AI-адаптер, обработка дневника, admin API, сценарии бота по CJM), следующий — 13.7 (job'ы scheduler с фейковыми таймерами). Покрытие проекта: 23.9% statements по `npm test`; интеграционные и сценарные наборы идут отдельной командой `npm run test:integration` и в это число не входят. Пилот (этап 12) — go/no-go после цикла когорты.
...
- [~] Этап 13 — Тестовое покрытие (13.1–13.6 готовы: инфраструктура, доставка в Telegram, AI-адаптер, дневник, admin API на живой БД, сценарии бота по CJM; дальше: job'ы → ИИ-уточнения и eval → порог покрытия)
