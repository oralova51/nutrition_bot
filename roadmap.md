...
**Текущий фокус:** Этап 13 — тестовое покрытие: шаг 13.1 (инфраструктура) завершён, следующий — 13.2 (доставка в Telegram). Пилот (этап 12) — go/no-go после цикла когорты.
...
- [~] Этап 13 — Тестовое покрытие (13.1 инфраструктура готова; дальше по убыванию риска: доставка → AI-адаптер → дневник → API → бот → job'ы → ИИ-уточнения и eval)
...
> Тестовая БД (для 13.5): отдельный Supabase-проект, адрес в `.env.test` как `TEST_DATABASE_URL`; помощник `requireTestDatabase()` падает с инструкцией, если он не задан.
...
ФТ-21
> Тесты на `sendTelegramMessageWithRetry` с `vi.useFakeTimers()`. Отдельно: убрать персональный алерт администратору из `sendTelegramMessage` (real-time путь бота без ретраев) — одиночные сбои закрывает job массовых сбоев (10.8); алерт при исчерпании 3 попыток остаётся по ФТ-21.
13.3 [ ]
Контрактные тесты адаптера AI на фикстурах ответов модели
ФТ-6, ФТ-24
> `openai-compatible-engine.ts`: битый JSON, `finish_reason: 'length'`, пустой `summaryText`, ошибка провайдера, повтор без `thinking: disabled`. SDK `openai` мокается, реальная модель не вызывается.
13.4 [ ]
processDiaryEntry на фейковом AIEngine
ФТ-6, ФТ-7
> Лимит 2–3 рекомендации в день, изоляция ошибок AI от подтверждения записи, вечерняя сводка не уходит дважды.
13.5 [ ]
