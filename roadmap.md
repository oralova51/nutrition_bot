...
**Текущий фокус:** Этап 3.15 в работе (приблизительные ответы без цифр). Шаг 3.14 (напоминание при брошенной анкете) отложён до этапа 4.1: выбор планировщика (node-cron / BullMQ) запланирован там, а параметр N для теста зафиксирован как 2 минуты.
...
- [~] Этап 3 — Подключение клиента (3A ссылки — код и admin API полностью реализованы и проверены (3.1–3.6); 3B онбординг+анкета — welcome и анкета 3.9–3.13 реализованы, 3.15 в работе; 3C настройки уведомлений — не начато)
...
> Реализуется в `packages/bot/src/services/onboarding.ts`: `parseNumberAnswer` учитывает `question.allowApproximate`, а number-вопросы `age` и `weight` из `packages/shared/src/questionnaire/template.ts` получают подсказки, что можно ответить примерно.
