...
**Текущий фокус:** Этап 13 — тестовое покрытие: шаги 13.1–13.5 завершены (инфраструктура, доставка в Telegram, AI-адаптер, обработка дневника, интеграционные тесты admin API), следующий — 13.6 (сценарные тесты бота по CJM). Покрытие проекта: 23.7% statements по `npm test`; интеграционный набор идёт отдельной командой `npm run test:integration` и в это число не входит. Пилот (этап 12) — go/no-go после цикла когорты.
...
- [~] Этап 13 — Тестовое покрытие (13.1–13.5 готовы: инфраструктура, доставка в Telegram, AI-адаптер, дневник, admin API на живой БД; дальше по убыванию риска: бот → job'ы → ИИ-уточнения и eval)
