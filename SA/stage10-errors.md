...
## 8. Алерт, если рекомендация или вечерняя сводка не собрались

Клиенту «тишина» на разовый treat или лимит 2–3 совета в день — это норма, администратора не будим. Будим, когда обработка **упала с ошибкой**:

- **Рекомендация не создана.** `processDiaryEntry` ловит сбой AI/БД, пишет `AI_ENGINE_ERROR` в лог и шлёт алерт. Типичные формулировки: закончились деньги на счёте (квота/биллинг), ключ отклонён, rate limit, провайдер недоступен.
- **Вечерняя сводка не собралась.** Job `evening-summary` собирает сбои за прогон в один дайджест. Если AI упал, но клиент получил эвристический запасной текст — в алерте это сказано прямо, чтобы не путать с «отчёта нет».
- Недоставка в Telegram **не дублируется**: её по-прежнему закрывает ФТ-21 / sender.
- Дедуп: не чаще одного алерта на класс ошибки за `JOB_FAILURE_ALERT_INTERVAL_MINUTES` (default 60). Биллинг/auth/rate limit — глобально, чтобы падение провайдера не слало по сообщению на каждого клиента.
- Реализация: `packages/shared/src/telegram/job-failure-alert.ts`, классификация `packages/shared/src/errors/provider-error.ts`.
