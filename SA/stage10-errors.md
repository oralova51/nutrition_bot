...
- Постоянные ошибки Telegram (`chat not found`, `403 forbidden` / bot blocked и аналогичные по тексту) **не ретраятся**: сразу `delivery_failed` и один алерт. Код `400` сам по себе не постоянный: туда же попадают битый HTML (`can't parse entities`) и слишком длинное сообщение — для них сначала пробуем отправить текст без разметки. Повторный алерт по тому же клиенту — не чаще раза в 24 ч. Окно считается по записям `Message` со статусом `delivery_failed` (не только по памяти процесса), чтобы рестарт scheduler не спамил администратора.
- Запись `Message` со статусом `sent` создаётся **только после** успешного `sendMessage`. Пока Telegram не подтвердил доставку (и на время retry), статус `delivery_failed` — иначе вечерняя сводка считает день уже отправленным и молчит.
