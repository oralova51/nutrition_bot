...
**Карта источников:** `vision.md` (продукт/метрики) · `UserResearch.md` (персоны) · `Domain_Analysis_v2.md` (сущности, бизнес-правила) · `ERD.md` (схема БД, поля, ограничения) · `Functional_Requirements.md` (ФТ-1..ФТ-24) · `nonFR.md` (нефункц. требования) · `CJM.md` (сценарий) · `anketa.md` (вопросы онбординга) · `adminAPI.md` (REST + Telegram Bot API) · `stage9-dashboards.md` (панели управления: админ и специалист, API-контракты + Postman) · `evening-summary.md` (ФТ-24 ежедневная вечерняя сводка + чек-лист/Postman) · `course-completion.md` (ФТ-13/14 завершение курса + ручной тест/Postman) · `stage12-pilot.md` (пилот: seed, чек-листы CJM, метрики, go/no-go).
...
- **Тон (ФТ-7):** как диетолог — без пересказа только что записанного; разовое удовольствие (пиво, десерт, джанк) — молчим; паттерн того же класса ≥ 2 за 7 локальных дней — частотная рамка, не замена на воду/протеин; обычную еду улучшаем парным советом к тому же выбору.
- **MVP-границы:** пилот на одной студии, один полный цикл курса (30 дней, когорта ~10 клиентов) — см. `stage12-pilot.md`.
- **Метрики пилота (пороги):** дневник ≥ 50%; соблюдение рекомендаций ≥ 50%; продления ≥ 50%; Feedback ≥ 4/5; досрочные отключения ≤ 20%; жалобы на навязчивость ≤ 2. Сбор: `npm run pilot:metrics`.
...
**Фаза 5 — Завершение:** ФТ-13 определение конца курса (Critical) · ФТ-14 итоговый отчёт клиенту и короткая сводка администратору в Telegram (Critical) · ФТ-15 запрос оценки 1-5 (High) · ФТ-16 при 1-3 звёздах — комментарий + уведомление админу (High) · ФТ-17 при 4-5 — благодарность + отзыв на 2Gis/Яндекс за бонус (High).
...
- **Playground ИИ-уточнений (ФТ-22):** `POST /internal/ai/clarity-check` `{description, firstName?, hasPhoto?, approxCalories?}`; auth Bearer admin-токен; в БД не пишет. Ответ: `needsClarification`, `missingFields`, `question`, `botReply` (текст клиенту), `provider` (`mock` \| `openai-compatible`). Postman: `postman/ai-clarity.postman_collection.json`.
- **Telegram Bot API (bot-сервис, не admin):** getMe/getUpdates/setWebhook (dev=polling, prod=webhook+secret_token); setMyCommands — публичное меню `/settings` (уведомления), `/site` (https://lpg39.ru), `/buy` (оплата абонемента, URL из `SUBSCRIPTION_CHECKOUT_URL`); `allowed_updates=["message","callback_query","my_chat_member"]`; sendMessage (parse_mode HTML), sendPhoto, sendChatAction(typing), editMessageText, answerCallbackQuery; getFile для фото (лимит 20 MB). `callback_data` ≤ 64 байт, примеры: `q:4:opt:2`, `settings:time:09:00`, `feedback:stars:4`, `pause:week`.
...
- **Telegram `/admin` (пилот):** allowlist `ADMIN_ALERT_TELEGRAM_ID`; кнопки «Пригласить клиента» (курс → имя → Client+Enrollment+deep link) и «Создать курс» (дни → авто-имя `Курс N дней`). Web-UI — post-MVP.
...
- **Scheduler internal (тест job'ов):** `POST /internal/jobs/daily-reminder|evening-reminder|evening-summary|course-completion` с `force=true` (+ опц. `clientId`); auth Bearer admin-токен. ФТ-24: `evening-summary.md`. ФТ-13/14 симуляция конца курса: `course-completion.md` (`force`+`clientId` игнорирует `endDate`). После `Report.type=final` администратору уходит короткая сводка в Telegram (`ADMIN_ALERT_TELEGRAM_ID`). После completion для повторного курса — `POST /admin/clients/:id/enrollments` (см. папку в `scheduler-jobs` Postman).

---

## 10. Ошибки и edge cases (stage10-errors.md)

- **ФТ-21:** Централизованный retry отправки в Telegram (3×5 мин) в `packages/shared/src/telegram/sender.ts`; постоянные ошибки (`chat not found`, bot blocked) без ретраев и с дедупом алерта 24 ч (по таблице `Message`, переживает рестарт процесса); статус `delivery_failed` на `Message`; уведомление администратора при исчерпании retry.
...
## 12. Пилот (stage12-pilot.md)

- **Seed:** `npm run db:seed:pilot` — курс 30 дней + 5 тестовых enrollment (happy / анкета / expired link / inactivity / user_stop).
- **Когорта:** ~10 реальных клиентов одной студии, цикл 30 дней (admin API).
- **Метрики:** `npm run pilot:metrics` — пороги §1 SA_SUMMARY (дневник/adherence/renewal ≥50%, Feedback ≥4, opt-out ≤20%, жалобы ≤2 через `PILOT_NAGGING_COMPLAINTS`).
- **Чек-листы:** happy path CJM 3–12 и альтернативные ветки — в `stage12-pilot.md`.
- **Go/no-go:** шаблон решения в `stage12-pilot.md` §6 после полного цикла.
