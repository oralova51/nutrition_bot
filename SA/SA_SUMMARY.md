...
- **Scheduler internal (тест job'ов):** `POST /internal/jobs/daily-reminder|evening-reminder|evening-summary|course-completion` с `force=true` (+ опц. `clientId`); auth Bearer admin-токен. ФТ-24: `evening-summary.md`. ФТ-13/14 симуляция конца курса: `course-completion.md` (`force`+`clientId` игнорирует `endDate`). После completion для повторного курса — `POST /admin/clients/:id/enrollments` (см. папку в `scheduler-jobs` Postman).

---

## 10. Ошибки и edge cases (stage10-errors.md)

- **ФТ-21:** Централизованный retry отправки в Telegram (3×5 мин) в `packages/shared/src/telegram/sender.ts`; постоянные ошибки (`chat not found`, bot blocked) без ретраев и с дедупом алерта 24 ч; статус `delivery_failed` на `Message`; уведомление администратора при исчерпании retry.
